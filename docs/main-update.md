...
- *优化更新时的体验*
- *优化Bypass链接的IP自动选择*
- *日志页面中增加"关于"用于显示依赖包名称及版本*

###### - 前端优化
- *细节优化*

## 0.6.89_20260405-164316_Beta
###### - 更新
- *增加多种去污染策略*
- *优化Bypass功能，提升稳定性与数据交互速度*
- *优化Bypass链接管理器逻辑*
...
## 0.6.89_20260326-054700_Beta

###### - 主要更新
- *增加使用内置命令手动更新root根文件*

###### - 前端优化
- *可在/path/gogdns/www/config/config.json中修改前端LOGO、壁纸、背景颜色、透明度或磨砂效果等。*
- *DNS 性能统计图表面板 的节点只在鼠标悬浮时显示 美化视觉*
- *取消Bypass面板按钮点击后的右下角提示*

###### - 陈旧的更新
- *增加首次运行时自设账号密码*
- *增加检测并自动更新*
- *使用SNI域前置链接时如果握手错误则降级为Roam漫游方式链接*
- *已自动支持PQC后量子加密，配置文件中增加后量子加密的开关参数*
- *增加实时显示当前Bypass的域前置与漫游链接列表并显示速度*
- *增加切换暗黑与明亮主题的功能*
- *配置文件中可设置DoH路由与WEB控制台路由的合并或分离*
- *主面板显示当前缓存的数量及内存占用*
- *可在控制台中使用内置命令手动压缩数据库*
- *删除web管理器中日志页面的"关于客户端配置文件"的说明，单个客户端转为使用面板配置*
- *重构Chrome插件*
- *配置文件中的服务不再使用影子值而采用以逗号分割的字符串形式*
- *组织服务列表重构采用静态表加KV库的形式*
- *启动时设置--debug-web参数来释放WEB数据进行前端调试*

## 0.6.62 20251215-130301 [Beta]
...
## 0.6.20 20250821-080559 [Beta] - 历史版本
